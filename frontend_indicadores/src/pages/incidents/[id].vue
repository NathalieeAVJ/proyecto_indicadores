<template>
  <v-container>
    <v-row>
      <v-col cols="12" md="8">
        <v-card>
          <v-card-title>Editar Incidencia #{{ route.params.id }}</v-card-title>
          <v-card-text>
...
const submit = async () => {
    ...
    try {
        ...
        await api.patch(`incidents/${route.params.id}/`, form.value);
          ...
          for (const item of usedParts.value) {
            if (item.spare_part && item.quantity_used > 0) {
              await api.post('spare-part-usage/', {
                ...
                notes: item.notes || `Usado en incidencia de teléfono #${route.params.id}`
              });
            }
          }
        }
...
onMounted(async () => {
    try {
        const [incRes, phonesRes, typesRes, usersRes, partsRes] = await Promise.all([
             api.get(`incidents/${route.params.id}/`),
             api.get('phones/'),
