  ...
  <v-container>
    <v-row class="mb-4">
      ...
      <v-col cols="12" md="4" class="text-right d-flex align-center justify-end">
        ...
        <v-btn color="primary" @click="openDialog()" prepend-icon="mdi-plus">Nuevo Ticket RB</v-btn>
      </v-col>
    </v-row>
    ...
    <v-card elevation="2">
      ...
      >
        ...
        <template v-slot:item.actions="{ item }">
          ...
          <v-btn icon size="small" variant="text" color="secondary" @click="createOT(item)" title="OT">
    ...
    <v-dialog v-model="dialog" max-width="800" persistent>
      <v-card>
        <v-card-title class="pa-4 text-h5 d-flex justify-space-between align-center">
          <span>{{ isEdit ? `Ticket RB #${editedId}` : 'Reportar Falla en Radiobase' }}</span>
        ...
        <v-card-text class="pa-6">
          <v-form @submit.prevent="saveItem" ref="formRef" id="rbIncidentForm">
            ...
            <v-textarea v-model="form.description" label="Descripción de la falla en infraestructura" rows="2"></v-textarea>
            ...
            <v-expand-transition>
              <div v-if="form.status === 'solved'">
                ...
                <h3 class="text-subtitle-1 font-weight-bold mb-3">Detalles de la Solución Técnica</h3>
                ...
                <v-card variant="outlined" class="mt-4">
                  <v-card-title class="text-subtitle-2 pa-2 bg-grey-lighten-4">
                    <v-icon size="small" class="me-2">mdi-toolbox-outline</v-icon>
                  ...
                  <v-card-text class="pa-2">
                    ...
                    <v-btn variant="text" size="small" color="primary" @click="usedParts.push({spare_part: null, quantity_used: 1})" class="mt-2">
                      <v-icon start>mdi-plus</v-icon> Agregar Material
                    </v-btn>
                  </v-card-text>
                </v-card>
              </div>
...
const headers = [
  ...
  { title: 'Radiobase', key: 'radio_base_detail.name' },
  { title: 'Código', key: 'radio_base_detail.code' },
...
const fetchIncidents = async () => {
  ...
  try {
    ...
    incidents.value = await api.get(`rb-incidents/?${params.toString()}`);
...
const loadMetadata = async () => {
  try {
    const [rb, f, u, sp] = await Promise.all([
      api.get('radio-bases/'), api.get('failure-types/'), api.get('users/'), api.get('spare-parts/')
    ]);
    radioBases.value = rb; failureTypes.value = f; users.value = u; spareParts.value = sp;
  } catch (e) { console.error(e); }
};
...
const openDialog = (item = null) => {
  ...
  if (item) {
    ...
    form.value = { title: '', radio_base: null, failure_type: null, description: '', status: 'pending', assigned_to: null, solution_comment: '', solved_date: '' };
  }
  dialog.value = true;
};
...
const saveItem = async () => {
  ...
  try {
    if (isEdit.value) {
      await api.patch(`rb-incidents/${editedId.value}/`, form.value);
      if (form.value.status === 'solved' && originalStatus.value !== 'solved') {
        for (const up of usedParts.value) {
          if (up.spare_part) {
            await api.post('spare-part-usage/', { ...up, incident_type: 'radiobase', incident_id: editedId.value });
          }
        }
      }
    } else {
...
const deleteItem = async (item) => {
  if (confirm(`¿Eliminar incidencia RB #${item.id}?`)) {
    await api.delete(`rb-incidents/${item.id}/`);
    fetchIncidents();
  }
};
...
const createOT = async (item) => {
  try {
    await api.post('work-orders/', { incident_type: 'radiobase', incident_id: item.id, assigned_to: item.assigned_to || authStore.user.id, status: 'assigned' });
    alert('Orden de Trabajo generada');
  } catch (e) { alert('Error al generar OT'); }
};

const exportPDF = async () => {
  const reportHeaders = ['ID', 'Asunto', 'Radiobase', 'Tipo Falla', 'Estado', 'Fecha'];
  const body = incidents.value.map(i => [i.id, i.title, i.radio_base_detail?.name, i.failure_type_detail?.name, i.status.toUpperCase(), formatDate(i.created_at)]);
  await generatePDFReport({ title: 'Reporte de Infraestructura (RB)', headers: reportHeaders, body });
};
